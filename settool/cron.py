...
def guidedtour_reminder(semester: Semester, today: datetime) -> None:
    ...
    if setting and setting.mail_reminder:
        ...
        for tour in semester.tour_set.filter(
            Q(date__day=lookup_day.day) & Q(date__month=lookup_day.month) & Q(date__year=lookup_day.year),
        ):
            participant: guidedtours.models.Participant
            for participant in [participant for participant in tour.participant_set if participant.on_the_tour]:
                setting.mail_reminder.send_mail_participant(participant)


def tutor_reminder(semester: Semester, today: datetime) -> None:
    ...
    if settings and settings.mail_reminder:
        ...
        for task in Task.objects.filter(
            Q(semester=semester)
            & Q(begin__day=lookup_day.day)
            & Q(begin__month=lookup_day.month)
            & Q(begin__year=lookup_day.year),
        ):
            tutor: Tutor
            for tutor in task.tutors:
                settings.mail_reminder.send_mail_task(tutor, task)


def fahrt_date_reminder(semester: Semester, today: datetime) -> None:
